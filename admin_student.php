...
/*
  if (!isset($res['error'])) {
    $query = "INSERT INTO Users (Nume, Email, Parola, Rol, Departament, Nota, AnStudiu, Materie, Credite) VALUES (:nume, :email, :parola, :rol, :departament, :nota, :anStudiu, :materie, :credite)"; 
    ...
    if ($check) {
      $res['success'][] = ['Student înscris cu succes!'];
...
if (!isset($res['error'])) {
  $query = "INSERT INTO Cat (Nume, Parola, Nota, Materie, Credite) VALUES (:nume, :parola, :nota, :materie, :credite)"; 
  $stmt = $conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
  $check = $stmt->execute([
    'nume' => $fullname,
    'parola' => password_hash($password, PASSWORD_DEFAULT),
...
<body>
  <div class="container">
    ...
    <form method="POST" id="form">
      ...
      <select id="year" style="height:50px;width:100%;margin: 5px 0px 20px 0px;">
